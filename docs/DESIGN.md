...
만들므로 원본에 없던 픽셀을 더할 수 있고, 그 다리가 ROI 밖을 지나는 경로가 실제로 존재한다:

| 조건 | 무작위 400회 중 유출 |
|---|---|
| 볼록 ROI(회전 사각형)가 **영상 프레임에 걸침** | **44건** |
| 오목 ROI(L자) | 5건 |
| 볼록 ROI가 프레임 안쪽에 완전히 포함 | 0건 |

프레임에 걸친 경우가 걸리는 이유는 OpenCV의 경계 처리다 — CLOSE의 침식 단계가 프레임 바깥을
채워진 것으로 보므로, 프레임 가장자리에서는 안쪽이라면 깎였을 픽셀이 살아남는다. ROI가 프레임에
잘리면 그 살아남은 픽셀이 ROI 폴리곤 밖에 놓인다. `rect_golden`이 회전 사각형이라 볼록이지만
**프레임에 걸리는 순간 이 경로가 열리므로, 마지막 컷은 지금도 실제로 필요하다.**
